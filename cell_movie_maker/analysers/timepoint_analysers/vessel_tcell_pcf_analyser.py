...
class VesselTCellPCFAnalyser(TimepointAnalyser):
    ...
    def analyse(self, timepoint:SimulationTimepoint, sim:Simulation=None)->pd.DataFrame|pd.Series|float|int:
        ...
        p1 = ms.query.interpret_query(ms.query.query(domain, "cell_type", "is", "Blood Vessel") & ms.query.query(domain, "target_radius", ">", 0))
    ...
    def __str__(self):
        return f'Vessel-TCell PCF r={self.r_max} dr={self.dr} step={self.step}'
